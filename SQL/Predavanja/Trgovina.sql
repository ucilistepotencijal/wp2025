...
-- Napraviti tablice kategorijeproizvodi i kategorije
CREATE TABLE kategorije (
    kategorija_id INT PRIMARY KEY AUTO_INCREMENT ID,
    naziv VARCHAR(100) NOT NULL,
    opis TEXT
)
CREATE TABLE kategorijeProizvodi (
    proizvod_id INT PRIMARY KEY AUTO_INCREMENT ID,
...
-- Popuniti tablicu kupci tako da imamo barem 2 kupca s istim imenom, kupca kojem ime počinje i završava samoglasnikom, 
-- počinje i završava suglasnikom, ima ime koje sadrži r i k zaredom, 
INSERT INTO kupci (ime) VALUES
('Marko'),    -- sadrži 'rk'
('Marko'),    -- isto ime kao prethodni (2 kupca s istim imenom)
('Ana'),      -- počinje i završava samoglasnikom
('Boris'),    -- počinje i završava suglasnikom
('Mirko');    -- ime sadrži "rk" zaredom

-- Popuniti tablicu proizvodi sa dostupnim i nedostupnim proizvodima
CREATE TABLE Proizvodi (
    proizvod_id INT PRIMARY KEY AUTO_INCREMENT ID,
    naziv VARCHAR(100) NOT NULL,
    cijena DECIMAL(10,2),
...
SELECT DISTINCT 'kategorija_id', naziv
FROM proizvodi;

SELECT COUNT(DISTINCT 'kategorija-id',naziv
FROM proizvodi;
