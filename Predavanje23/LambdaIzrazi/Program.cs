...
Console.WriteLine("Broj imena koja sadrže slovo o: " + imenaSaSlovomO);

int[] brojevi = { 1, 3, 5, 15, 9, 24 };
var kvadrati = brojevi.Select(i => i * i).ToList();
string rezultat5 = "";
kvadrati.ForEach(i => rezultat5 += (i + ", "));
Console.WriteLine(rezultat5);

IEnumerable<int> sintaksaUpita = from b in brojevi where b % 2 == 0 orderby b select b;

IEnumerable<int> sintaksaMetode = brojevi.Where(b => b % 2 == 0).OrderBy(b => b);
